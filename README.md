...
- 流水线会生成 MSIX 包。如果配置了 `WINDOWS_SIGNING_CERT_BASE64` 和 `WINDOWS_SIGNING_CERT_PASSWORD` 两个仓库 secrets，会使用正式证书签名；否则自动生成测试签名证书，并随 Release 上传 `.cer`。

没有正式代码签名证书时，Windows 仍可能出现安全提示。要减少 SmartScreen 提示，需要使用可信 CA 签发的代码签名证书，并在 GitHub secrets 中配置 PFX 证书内容和密码。测试签名证书只建议用于自己电脑或可信内网验证。
