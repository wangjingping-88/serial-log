...
- 不同测试使用独立工作区，分别保存窗口、命令、主题、快捷键、日志和协作配置。
- 切换工作区不停止后台采集、保存和循环发送；测试菜单只作用于当前工作区。
- 支持新建、复制、重命名、停止与删除；只剩一个工作区时禁止删除。
- 详细使用与迁移说明见 [测试工作区与按需协作](docs/test-workspaces.md)。
...
- 工作区、页面、协作、主题、视图、日志、快捷键和帮助集中到标题栏菜单。
...
- 采用主机中继架构，不建立从机之间的直连。
- 窗口默认不共享、不订阅；在来源端勾选“共享”，接收端通过“协作 → 订阅窗口”选择并保存。
- 主机可查看并控制已订阅的从机窗口；从机可只读查看已订阅的主机和其他从机窗口。
- 取消共享保留对方窗口及历史，恢复后自动继续；取消订阅才移除窗口。
- 每台电脑不会显示自己的镜像窗口。
...
3. 在“日志”菜单选择保存目录，按需新建日志批次。
4. 需要发送命令时，在“视图”菜单显示并停靠命令区。
5. 需要多机共享时，设置主机或从机模式，来源端开启共享，接收端选择订阅并保存。

日志默认保存到：

```text
<工具目录>\data\logs\<工作区ID>\<日期>\<采集批次>
```

点击“新建日志批次”后，当前工作区已连接窗口的后续日志切换到新目录，其他工作区不受影响。自定义日志根目录仍可使用。
...
3. 各来源端勾选窗口“共享”，接收端打开“订阅窗口”勾选需要的窗口并保存；新订阅窗口可能位于后续页面。
4. 主机可向从机远端窗口发送命令；从机侧远端窗口只读。
5. 确保局域网互通，且 Windows 防火墙允许主机监听端口。

参与方必须使用协议 3。不要将带有相同电脑 ID 的 `data` 直接复制到另一台电脑协作，否则可能将对方当作自己过滤。跨电脑复制后的身份冲突目前需要手动处理。
