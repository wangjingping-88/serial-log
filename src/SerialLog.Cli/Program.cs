...
public static class Program
{
    ...
    public static async Task<int> MainAsync(string[] args)
    {
        ...
        return args[0] switch
        {
            ...
            "ecolink-loop" => await RunEcoLinkLoopAsync(args.Skip(1).ToArray()).ConfigureAwait(false),
            "ecolink-ota" => await RunEcoLinkOtaAsync(args.Skip(1).ToArray()).ConfigureAwait(false),
            _ => Unknown(args[0])
        };
    }

    private static async Task<int> RunEcoLinkOtaAsync(string[] args)
    {
        if (args.Any(IsHelp))
        {
            PrintUsage();
        ...
        {
            Console.Error.WriteLine("ecolink-ota需要--config参数。");
            PrintUsage();
            return 1;
        }
        ...
        var config = JsonSerializer.Deserialize<EcoLinkOtaRunConfig>(
            json,
            new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
        if (config is null)
        {
            Console.Error.WriteLine($"配置文件无效：{configPath}");
            return 1;
        }
        ...
        try
        {
            var runner = new EcoLinkOtaRunner(config);
    ...
    private static async Task<int> RunEcoLinkLoopAsync(string[] args)
    {
        var options = ParseOptions(args);
        if (!options.TryGetValue("--config", out var configPath))
        {
            Console.Error.WriteLine("ecolink-loop 需要 --config。");
            PrintUsage();
            return 1;
        }
        ...
        var json = await File.ReadAllTextAsync(configPath).ConfigureAwait(false);
        var config = JsonSerializer.Deserialize<EcoLinkLoopRunConfig>(
            json,
            new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
        if (config is null)
        {
        ...
        var runner = new EcoLinkLoopRunner(config);
    ...
    private static void PrintUsage()
    {
        ...
        Console.WriteLine("  SerialLog.Cli ecolink-loop --config <ecolink-loop.json>");
        Console.WriteLine("  SerialLog.Cli ecolink-ota --config <ecolink-ota.json>");
        Console.WriteLine();
        ...
        Console.WriteLine("  SerialLog.Cli ecolink-loop --config docs\\ecolink-loop-config.example.json");
        Console.WriteLine("  SerialLog.Cli ecolink-ota --config docs\\ecolink-ota-config.example.json");
    }
}
